...
struct async_write_messages_implementation
{
  ...
  void operator()(Self& self,
      ...
      std::size_t)
  {
    std::cout << "bbbbbbbbbbbbbbbbbbb" << state_ << std::endl;
    if (!error)
    {
      switch (state_)
      {
  ...
  {
    std::cout << "AAAAAAAAAAAAAAAAAAA" << state_ << std::endl;
    if (!error)
    {
      switch (state_)
      {
      ...
      case waiting:
        ...
        boost::asio::async_write(
            socket_, boost::asio::buffer(*encoded_message_), std::move(self));
        return; // Composed operation not yet complete.
      }
    }
    // This point is reached only on completion of the entire composed
...
TEST_CASE("Boost completion handler")
{
//   test_callback();
  test_future();
